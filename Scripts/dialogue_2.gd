...
func _ready() -> void:
	...
	while true:
		...
		if dialogue_label.has_method("type_out"):
			...
			var prev_mouse_pressed: bool = Input.is_mouse_button_pressed(MouseButton.LEFT)
			while dialogue_label.is_typing:
				...
				var mouse_pressed: bool = Input.is_mouse_button_pressed(MouseButton.LEFT)
		...
		if input_pressed_during_typing:
			...
			var prev_mouse: bool = Input.is_mouse_button_pressed(MouseButton.LEFT)
			while true:
				await get_tree().process_frame
				if Input.is_action_just_pressed("ui_accept"):
					break
				var mouse_now: bool = Input.is_mouse_button_pressed(MouseButton.LEFT)
